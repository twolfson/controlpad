...
function windowInfo(id, cb) {
  ...
  exec('xwininfo -id ' + id, function handleInfo (err, _info) {
    ...
    console.log('wat', id);
    console.log('xxxxx', grep(_infoLines, 'Width').split(/ /g));
    var info = {
      width: +grep(_infoLines, 'Width').split(/ /g)[3],
      height: +grep(_infoLines, 'Height').split(/ /g)[3],
      left: +grep(_infoLines, 'Absolute upper-left X').split(/ /g)[6],
      top: +grep(_infoLines, 'Absolute upper-left Y').split(/ /g)[6]
    };

    exec('xprop -id ' + id + ' | grep _NET_WM_STATE', function handleState (err, state) {
      // If there was an error, callback with it
      if (err) {
        return cb(err);
      }

      // Otherwise, determine if we are maximized
      var isHorzMaximized = state.match('_NET_WM_STATE_MAXIMIZED_HORZ');
